...
function RouterProvider(_ref) {
  ...
  }, state.initialized || router2.future.v7_partialHydration ? React2.createElement(DataRoutes, {
...
function convertRoutesToDataRoutes2(routes, mapRouteProperties2, parentPath, manifest) {
  ...
  return routes.map((route, index) => {
    let treePath = [...parentPath, index];
    let id = typeof route.id === "string" ? route.id : treePath.join("-");
    invariant3(route.index !== true || !route.children, "Cannot specify children on an index route");
    invariant3(!manifest[id], 'Found a route id collision on id "' + id + `".  Route id's must be globally unique within Data Router usages`);
...
function matchRouteBranch2(branch, pathname) {
  ...
  for (let i = 0; i < routesMeta.length; ++i) {
    ...
    matches.push({
      ...
      pathnameBase: normalizePathname2(joinPaths3([matchedPathname, match.pathnameBase])),
      route
    });
    if (match.pathnameBase !== "/") {
...
function resolveTo2(toArg, routePathnames, locationPathname, isPathRelative) {
  ...
  if (typeof toArg === "string") {
    ...
    to = _extends5({}, toArg);
    invariant3(!to.pathname || !to.pathname.includes("?"), getInvalidPathError2("?", "pathname", "search", to));
    invariant3(!to.pathname || !to.pathname.includes("#"), getInvalidPathError2("#", "pathname", "hash", to));
    invariant3(!to.search || !to.search.includes("#"), getInvalidPathError2("#", "search", "hash", to));
  }
  let isEmptyPath = toArg === "" || to.pathname === "";
  let toPathname = isEmptyPath ? "/" : to.pathname;
  let from;
...
function createRouter3(init) {
  ...
  function initialize() {
    ...
    if (!state.initialized) {
      startNavigation(Action3.Pop, state.location, {
  ...
  function completeNavigation(location, newState, _temp) {
    ...
    if (isUninterruptedRevalidation)
      ;
    else if (pendingAction === Action3.Pop)
      ;
    else if (pendingAction === Action3.Push) {
      init.history.push(location, location.state);
    } else if (pendingAction === Action3.Replace) {
  ...
  async function navigate(to, opts) {
    ...
    let historyAction = Action3.Push;
    if (userReplace === true) {
      historyAction = Action3.Replace;
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, flushSync, submission) {
    ...
    if (state.navigation.state === "loading" && loadId > pendingNavigationLoadId) {
      invariant3(pendingAction, "Expected pending action");
      pendingNavigationController && pendingNavigationController.abort();
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, flushSync, submission) {
    ...
    invariant3(!isDeferredResult(result), "Unhandled fetcher deferred data");
  ...
  async function startRedirectNavigation(state2, redirect4, _temp2) {
    ...
    invariant3(redirectLocation, "Expected a location on the redirect navigation");
    if (isBrowser2) {
      let isDocumentReload = false;
      if (redirect4.reloadDocument) {
        isDocumentReload = true;
      } else if (ABSOLUTE_URL_REGEX2.test(redirect4.location)) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace === true ? Action3.Replace : Action3.Push;
    let {
      formMethod,
      formAction,
      formEncType
...
function normalizeTo(location, matches, basename, prependBasename, to, v7_relativeSplatPath, fromRouteId, relative) {
  ...
  let path = resolveTo2(to ? to : ".", getResolveToMatches2(contextualMatches, v7_relativeSplatPath), stripBasename3(location.pathname, basename) || location.pathname, relative === "path");
  if (to == null) {
    path.search = location.search;
...
function getMatchesToLoad(history, state, matches, submission, location, isInitialLoad, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError) {
  ...
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    return shouldRevalidateLoader(match, _extends5({
      currentUrl,
  ...
  fetchLoadMatches.forEach((f, key) => {
    ...
    if (fetchRedirectIds.has(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader(fetcherMatch, _extends5({
        currentUrl,
...
async function callLoaderOrAction(type, request, match, matches, manifest, mapRouteProperties2, basename, v7_relativeSplatPath, opts) {
  ...
  if (isResponse(result)) {
    ...
    if (redirectStatusCodes.has(status)) {
      ...
      invariant3(location, "Redirects returned/thrown from loaders/actions must have a Location header");
...
function processRouteLoaderData(matches, matchesToLoad, results, pendingError, activeDeferreds) {
  ...
  results.forEach((result, index) => {
    ...
    invariant3(!isRedirectResult(result), "Cannot handle redirect results in processLoaderData");
    if (isErrorResult(result)) {
  ...
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    ...
    if (controller && controller.signal.aborted) {
      ...
      if (!(errors && errors[boundaryMatch.route.id])) {
        errors = _extends5({}, errors, {
          [boundaryMatch.route.id]: result.error
        });
      }
      state.fetchers.delete(key);
    } else if (isRedirectResult(result)) {
      invariant3(false, "Unhandled fetcher revalidation redirect");
    } else if (isDeferredResult(result)) {
      invariant3(false, "Unhandled fetcher deferred data");
    } else {
...
async function resolveDeferredResults(currentMatches, matchesToLoad, results, signals, isFetcher, currentLoaderData) {
  for (let index = 0; index < results.length; index++) {
    ...
    if (isDeferredResult(result) && (isFetcher || isRevalidatingLoader)) {
      ...
      invariant3(signal, "Expected an AbortSignal for revalidating fetcher deferred result");
...
function useDataRouterContext3() {
  ...
  invariant4(context, "You must render this element inside a <DataRouterContext.Provider> element");
  return context;
}
function useDataRouterStateContext() {
  let context = React3.useContext(DataRouterStateContext);
  invariant4(context, "You must render this element inside a <DataRouterStateContext.Provider> element");
  return context;
}
var RemixContext = React3.createContext(void 0);
RemixContext.displayName = "Remix";
function useRemixContext() {
  let context = React3.useContext(RemixContext);
  invariant4(context, "You must render this element inside a <Remix> element");
  return context;
}
...
var NavLink2 = React3.forwardRef(({
  to,
  prefetch = "none",
  ...props
}, forwardedRef) => {
  ...
  return React3.createElement(React3.Fragment, null, React3.createElement(NavLink, _extends6({}, props, prefetchHandlers, {
    ref: mergeRefs(forwardedRef, ref),
    to
  })), shouldPrefetch && !isAbsolute ? React3.createElement(PrefetchPageLinks, {
    page: href
  }) : null);
});
NavLink2.displayName = "NavLink";
var Link2 = React3.forwardRef(({
  to,
  prefetch = "none",
  ...props
}, forwardedRef) => {
  ...
  let href = useHref(to);
  let [shouldPrefetch, ref, prefetchHandlers] = usePrefetchBehavior(prefetch, props);
  return React3.createElement(React3.Fragment, null, React3.createElement(Link, _extends6({}, props, prefetchHandlers, {
    ref: mergeRefs(forwardedRef, ref),
    to
  })), shouldPrefetch && !isAbsolute ? React3.createElement(PrefetchPageLinks, {
    page: href
...
function Links() {
  ...
  return React3.createElement(React3.Fragment, null, criticalCss ? React3.createElement("style", {
    ...
    key,
    link
  }) => isPageLinkDescriptor(link) ? React3.createElement(PrefetchPageLinks, _extends6({
    key
  }, link)) : React3.createElement("link", _extends6({
    key
  }, link))));
}
function PrefetchPageLinks({
  ...
  return React3.createElement(PrefetchPageLinksImpl, _extends6({
    page,
    matches
  }, dataLinkProps));
}
...
function PrefetchPageLinksImpl({
  ...
  return React3.createElement(React3.Fragment, null, dataHrefs.map((href) => React3.createElement("link", _extends6({
    key: href,
    rel: "prefetch",
    as: "fetch",
    href
  }, linkProps))), moduleHrefs.map((href) => React3.createElement("link", _extends6({
    key: href,
    rel: "modulepreload",
    href
    ...
    React3.createElement("link", _extends6({
      key
    }, link))
  )));
}
function Meta() {
  ...
  return React3.createElement(React3.Fragment, null, meta.flat().map((metaProps) => {
    if (!metaProps) {
      return null;
    }
    if ("tagName" in metaProps) {
      ...
      let Comp = tagName;
      return React3.createElement(Comp, _extends6({
        key: JSON.stringify(rest)
      }, rest));
    }
    if ("title" in metaProps) {
      return React3.createElement("title", {
        key: "title"
    ...
    if ("charSet" in metaProps && metaProps.charSet != null) {
      return typeof metaProps.charSet === "string" ? React3.createElement("meta", {
        key: "charSet",
    ...
    return React3.createElement("meta", _extends6({
...
function Await2(props) {
  return React3.createElement(Await, props);
}
    ...
    contextScript += !activeDeferreds ? "" : ["__remixContext.p = function(v,e,p,x) {", "  if (typeof e !== 'undefined') {", true ? "    x=new Error(e.message);\n    x.stack=e.stack;" : '    x=new Error("Unexpected Server Error");\n    x.stack=undefined;', "    p=Promise.reject(x);", "  } else {", "    p=Promise.resolve(v);", "  }", "  return p;", "};", "__remixContext.n = function(i,k) {", "  __remixContext.t = __remixContext.t || {};", "  __remixContext.t[i] = __remixContext.t[i] || {};", "  let p = new Promise((r, e) => {__remixContext.t[i][k] = {r:(v)=>{r(v);},e:(v)=>{e(v);}};});", typeof abortDelay === "number" ? `setTimeout(() => {if(typeof p._error !== "undefined" || typeof p._data !== "undefined"){return;} __remixContext.t[i][k].e(new Error("Server timeout."))}, ${abortDelay});` : "", "  return p;", "};", "__remixContext.r = function(i,k,v,e,p,x) {", "  p = __remixContext.t[i][k];", "  if (typeof e !== 'undefined') {", true ? "    x=new Error(e.message);\n    x.stack=e.stack;" : '    x=new Error("Unexpected Server Error");\n    x.stack=undefined;', "    p.e(x);", "  } else {", "    p.r(v);", "  }", "};"].join("\n") + Object.entries(activeDeferreds).map(([routeId, deferredData]) => {
      ...
      let promiseKeyValues = deferredData.deferredKeys.map((key) => {
        if (pendingKeys.has(key)) {
          deferredScripts.push(React3.createElement(DeferredHydrationScript, {
            key: `${routeId} | ${key}`,
            deferredData,
            routeId,
            dataKey: key,
...
import(${JSON.stringify(manifest.entry.module)});`;
    return React3.createElement(React3.Fragment, null, React3.createElement("script", _extends6({}, props, {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: void 0
    })), React3.createElement("script", _extends6({}, props, {
      suppressHydrationWarning: true,
  ...
  let nextMatches = React3.useMemo(() => {
    if (navigation.location) {
      ...
      invariant4(matches2, `No routes match path "${navigation.location.pathname}"`);
      return matches2;
    }
    return [];
  ...
  return isHydrated ? null : React3.createElement(React3.Fragment, null, React3.createElement("link", {
    rel: "modulepreload",
    href: manifest.url,
    crossOrigin: props.crossOrigin
  }), React3.createElement("link", {
    rel: "modulepreload",
    href: manifest.entry.module,
    crossOrigin: props.crossOrigin
  }), dedupe(preloads).map((path) => React3.createElement("link", {
    key: path,
    rel: "modulepreload",
    href: path,
    crossOrigin: props.crossOrigin
...
function DeferredHydrationScript({
  ...
  if (typeof document === "undefined" && deferredData && dataKey && routeId) {
    invariant4(deferredData.pendingKeys.includes(dataKey), `Deferred data for route ${routeId} with key ${dataKey} was not pending but tried to render a script for it.`);
  }
  return React3.createElement(React3.Suspense, {
    fallback: (
      // This makes absolutely no sense. The server renders null as a fallback,
      // but when hydrating, we need to render a script tag to avoid a hydration issue.
      // To reproduce a hydration mismatch, just render null as a fallback.
      typeof document === "undefined" && deferredData && dataKey && routeId ? null : React3.createElement("script", _extends6({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
          __html: " "
  ...
  }, typeof document === "undefined" && deferredData && dataKey && routeId ? React3.createElement(Await2, {
    resolve: deferredData.data[dataKey],
    errorElement: React3.createElement(ErrorDeferredHydrationScript, {
      dataKey,
      routeId,
      scriptProps,
      serializeError
    ...
    children: (data) => {
      return React3.createElement("script", _extends6({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
  ...
  }) : React3.createElement("script", _extends6({}, scriptProps, {
    async: true,
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
      __html: " "
    ...
    void (async () => {
      try {
        ...
        for (let [key, resolver] of Object.entries(deferredResolvers)) {
          resolver.reject(new AbortedDeferredError2(`Deferred ${key} will never be resolved`));
        }
      } catch (error) {
...
function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), spaModeLazyPromise = Promise.resolve({
  ...
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    invariant4(routeModule, "No `routeModule` available to create server routes");
    let dataRoute = {
...
function preventInvalidServerHandlerCall(type, route, isSpaMode) {
  if (isSpaMode) {
    ...
    console.error(msg2);
    throw new ErrorResponseImpl2(400, "Bad Request", new Error(msg2), true);
  }
  let fn = type === "action" ? "serverAction()" : "serverLoader()";
  let msg = `You are trying to call ${fn} on a route that does not have a server ${type} (routeId: "${route.id}")`;
  if (type === "loader" && !route.hasLoader || type === "action" && !route.hasAction) {
    console.error(msg);
    throw new ErrorResponseImpl2(400, "Bad Request", new Error(msg), true);
  }
}
function noActionDefinedError(type, routeId) {
  ...
  console.error(msg);
  throw new ErrorResponseImpl2(405, "Method Not Allowed", new Error(msg), true);
}
    ...
    if (routeModule) {
      ...
      dataRoute.loader = async ({
        ...
        try {
          let result = await prefetchStylesAndCallHandler(async () => {
            invariant4(routeModule, "No `routeModule` available for critical-route loader");
      ...
      dataRoute.action = ({
        ...
        return prefetchStylesAndCallHandler(async () => {
          invariant4(routeModule, "No `routeModule` available for critical-route action");
...
function RemixBrowser(_props) {
  if (!router) {
    ...
    if (initialPathname !== hydratedPathname && !window.__remixContext.isSpaMode) {
      ...
      window.location.reload();
      return React7.createElement(React7.Fragment, null);
    }
  ...
  return React7.createElement(RemixContext.Provider, {
    value: {
...
function ScrollRestoration2({
  ...
  return React8.createElement("script", _extends6({}, props, {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
function StaticRouterProvider({
  ...
  !(router2 && context) ? true ? invariant2(false, "You must provide `router` and `context` to <StaticRouterProvider>") : invariant2(false) : void 0;
  ...
  }, React9.createElement(DataRoutes2, {
    routes: router2.routes,
    future: router2.future,
    state
...
function RemixServer({
  ...
  return React10.createElement(RemixContext.Provider, {
    value: {
      manifest,
      routeModules,
      criticalCss,
